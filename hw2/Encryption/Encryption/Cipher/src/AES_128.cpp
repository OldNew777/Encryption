#include "AES_128.h"
#include "DataGenerator.h"

#include <omp.h>
#include <assert.h>
#include <fstream>

using namespace std;


const uint8_t SBox[256] = {
	,
};
const uint8_t SBoxInv[256] = {
	,
};


bool AES_128::PrintFlag = false;

void AES_128::ExpandKey(uint8_t key[16]) {
	memcpy(expansiveKey.bytes, key, 16);

	for (int i = 4; i < 44; ++i) {
		if (i & 0b11)
			expansiveKey.words[i] = expansiveKey.words[i - 4] ^ expansiveKey.words[i - 1];
		else
			expansiveKey.words[i] = expansiveKey.words[i - 4] ^ T(expansiveKey.words[i - 1], i >> 2);
	}
}

void AES_128::CipherBlock(uint8_t key[16], uint8_t* src, uint8_t* dst)
{
	AES_128 aes;
	aes.ExpandKey(key);
	aes.Cipher(src, dst);
	aes.Decipher(dst, src);
}

AES_128& AES_128::operator=(const AES_128& src)
{
	expansiveKey = src.expansiveKey;
	return *this;
}

uint8_t* AES_128::Encrypt(uint8_t* src, uint32_t lenSrc, uint32_t& lenDst, uint8_t key[16], uint8_t IV[16])
{
	AES_128 aes_128;

	aes_128.ExpandKey(key);

	lenDst = (lenSrc & ~15) + 16;


	uint8_t
		* dst = new uint8_t[lenDst],
		padding = 16 - (lenSrc & 15);


	memcpy(dst, src, lenSrc);
	memset(dst + lenSrc, padding, lenDst - lenSrc);

	for (int i = 0; i < 16; ++i)
		dst[i] ^= IV[i];
	aes_128.Cipher(dst, dst);
	for (int i = 16; i < lenDst; i += 16) {
		//for (int j = 0; j < 16; ++j)
		//	dst[i + j] ^= dst[i - 16 + j];

		uint64_t* dstptr = (uint64_t*)(dst + i);
		uint64_t
			& dst1 = *dstptr,
			& dst2 = *(dstptr + 1),
			dstLast1 = *(dstptr - 2),
			dstLast2 = *(dstptr - 1);
		dst1 ^= dstLast1;
		dst2 ^= dstLast2;

		aes_128.Cipher(dst + i, dst + i);
	}

	if (PrintFlag)
		printf("Encrypt::padding = 0x%02x\n", int(padding));

	return dst;
}

uint8_t* AES_128::Decrypt(uint8_t* src, uint32_t lenSrc, uint32_t& lenDst, uint8_t key[16], uint8_t IV[16])
{
	assert(((lenSrc & 15) == 0) && (lenSrc >= 16));

	AES_128 aes_128;
	aes_128.ExpandKey(key);

	uint8_t* data = new uint8_t[lenSrc];
	memcpy(data, src, lenSrc);

#pragma omp parallel for
	for (int i = 0; i < lenSrc; i += 16) {
		AES_128 aes_128_t = aes_128;
		aes_128_t.Decipher(data + i, data + i);
	}

	for (int i = 0; i < 16; ++i)
		data[i] ^= IV[i];
	uint64_t* srcptr = (uint64_t*)src;
#pragma omp parallel for
	for (int i = 16; i < lenSrc; i += 16) {
		//for (int j = 0; j < 16; ++j)
		//	data[i + j] ^= src[i - 16 + j];

		uint64_t
			* dataptr = (uint64_t*)(data + i);
		uint64_t
			& dst1 = *dataptr,
			& dst2 = *(dataptr + 1);
		dst1 ^= srcptr[(i >> 3) - 2];
		dst2 ^= srcptr[(i >> 3) - 1];
	}

	if (PrintFlag)
		printf("Decrypt::padding = 0x%02x\n", int(data[lenSrc - 1]));

	lenDst = lenSrc - data[lenSrc - 1];
	uint8_t* dst = new uint8_t[lenDst];
	memcpy(dst, data, lenDst);

	delete[] data;
	return dst;
}
